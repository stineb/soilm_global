...
# pdf( "fig/bias_vs_fvar_ALL.pdf", width=sum(widths), height=sum(heights) )
...
##------------------------------------------------
## GPPobs/GPPmod vs. soil moisture
##------------------------------------------------
magn <- 3
ncols <- 4
nrows <- 3
...
pdf( "fig/bias_vs_soilm_ALL.pdf", width=sum(widths), height=sum(heights) )
  ...
  #---------------------------------------------------------
    ...
    ## use only data during droughts for stats
    sub <- filter( df_dday_agg, is_drought_byvar==1 ) 
    stats <- analyse_modobs( sub$ratio_obs_mod, sub$soilm_mean, do.plot=FALSE )
