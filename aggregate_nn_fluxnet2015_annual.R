...
##------------------------------------------------
plot_spatial_annual_s0 <- function( adf, meandf, linmod_list_pmodel, linmod_mean_pmodel, stats_adf, stats_meandf, filn=NA ){

  if (!is.na(filn)) pdf(filn, width = 8, height = 6)
    linmod_mean_pmodel <- lm( gpp_pmodel ~ gpp_obs, data = meandf )
    ...
    for (sitename in ls(linmod_list_pmodel) ){
      ...
      if (length(linmod_list_pmodel[[ sitename ]]$fitted.values)>3){
        par(las=1)
        lines( tmp$gpp_obs[ as.numeric(names(linmod_list_pmodel[[ sitename ]]$fitted.values)) ], linmod_list_pmodel[[ sitename ]]$fitted.values, col="black")
      }
    }
    lines( c(0,5000), c(0,5000), lty=3 )
...
plot_spatial_annual_s0( adf, meandf, linmod_list_pmodel_s0,  linmod_mean_pmodel_s0,  stats_adf, stats_meandf, filn="fig/modobs_ann_spatial_temporal_pmodel_s0.pdf"  )


##------------------------------------------------
## P-MODEL s1a: Plot correlation of mean per site
##------------------------------------------------
plot_spatial_annual_s1 <- function( adf, meandf, linmod_list_pmodel, linmod_mean_pmodel, stats_adf, stats_meandf, filn=NA ){
  if (!is.na(filn)) pdf(filn, width = 8, height = 6)
...
plot_spatial_annual_s1( adf, meandf, linmod_list_pmodel_s1b,  linmod_mean_pmodel_s1b,  stats_adf, stats_meandf, filn="fig/modobs_ann_spatial_temporal_pmodel_s1b.pdf"  )
