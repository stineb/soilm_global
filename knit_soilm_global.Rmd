...
```{r align, warning=FALSE, eval=TRUE}
...
df_coef <- tibble( approach="I", apar=coef(linearfit_I$linmod)["(Intercept)"], bpar=coef(linearfit_I$linmod)["meanalpha"], apar_grass=NA, bpar_grass=NA )
```
...
ddf <- ddf %>%  mutate( flue_est_I = calc_flue_est_alpha( soilm_mean, alpha=meanalpha, apar=coef(linearfit_I$linmod)[1], bpar=coef(linearfit_I$linmod)[2], cpar=0.125, dpar=0.75 ) ) %>% 
...
df_coef <- df_coef %>% bind_rows( tibble( approach="II", apar=coef(linearfit_II$linmod)["(Intercept)"], bpar=coef(linearfit_II$linmod)["meanalpha"], apar_grass=NA, bpar_grass=NA ))
```

Again, we have a linear relationship between the maximum LUE reduction (y-axis intersect) and mean site aridity (AET/PET).
```{r}
par(las=1)
with(linearfit_II$data, plot( meanalpha, y0, pch=16 ) )
abline( linearfit_II$linmod )
mtext( line=-1.5, bquote( italic(R)^2 == .(format( summary( linearfit_II$linmod )$r.squared, digits = 2) ) ),  adj=0.1, cex=1 )
cf <- coef(linearfit_II$linmod) %>% round( 2 )
...
df_coef <- df_coef %>% bind_rows( tibble( approach="III", apar=coef(linearfit_III$linmod)["(Intercept)"], bpar=coef(linearfit_III$linmod)["meanalpha"], apar_grass=NA, bpar_grass=NA ))
```
...
df_coef <- df_coef %>% bind_rows( tibble( approach="IV", apar=coef(linearfit_IV$linmod_tree)["(Intercept)"], bpar=coef(linearfit_IV$linmod_tree)["meanalpha"], apar_grass=coef(linearfit_IV$linmod_grass)["(Intercept)"], bpar_grass=coef(linearfit_IV$linmod_grass)["meanalpha"] ))
```

Again, we have a linear relationship between the maximum LUE reduction (y-axis intersect) and mean site aridity (AET/PET).
```{r}
par(las=1)
with( filter( linearfit_IV$data, classid %in% c("GRA", "CSH")), plot( meanalpha, y0, pch=16 , ylim=c(0,1), xlim=c(0,1)) )
...
mtext( line=-1.5, bquote( italic(R)^2 == .(format( summary( linearfit_IV$linmod_grass )$r.squared, digits = 2) ) ),  adj=0.1, cex=1 )
cf <- coef(linearfit_IV$linmod_grass) %>% round( 2 )
eq <- paste0( "y = ", cf[1], ifelse(sign(cf[2])==1, " + ", " - "), abs(cf[2]), " x " )
mtext( line=-2.5, eq, adj=0.1 )
```
...
<!-- ```{r plot_linearfit_II_ratio, warning=FALSE, message=FALSE, echo=FALSE} -->
<!-- source("plot_linearfit_II.R") -->
<!-- load("data/linearfit_I.Rdata") -->

<!-- ## plot fLUE0 vs. mean alpha and functional relationship (data cloud and fitted line) for all sites. -->
<!-- # plot_linearfit_II( linearfit_II, linearfit_I, df=ddf ) -->
<!-- ``` -->
...
- `s1b`: with additional soil moisture stress, following parametrisation IV
...
```{r echo=FALSE}
nc <- nc_open( paste0( myhome, "/data/pmodel_fortran_output/v2/gpp_pmodel_s0_RELVAR_GLOB.nc" ) )
relvar_s0 <- ncvar_get( nc, varid="gpp" )
nc_close(nc)

nc <- nc_open( paste0( myhome, "/data/pmodel_fortran_output/v2/gpp_pmodel_s1_RELVAR_GLOB.nc" ) )
