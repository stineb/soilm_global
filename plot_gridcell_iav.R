...
## straight axes
par(las=1)
plot(  density( var$ISAM ), col="red", type="n", xlab="GPP variance (kgC/m2/yr)", xlim=c(0,40), ylim=c(0,0.4), main="" )
...
# ## logarithmic y axis
# par(las=1)
# plot(  density( var$ISAM ), col="red", type="n", lwd=2, xlab="GPP variance (kgC/m2/yr)", log="y", xlim=c(0,40), ylim=c(1e-4, 5e-1) )
# lapply( seq(length(modls)), function(x) lines( density( var[[ x ]] ), col=cols[x], lwd=lwds[x], lty=ltys[x] ) )
# legend( "topright", modls, col=cols, lwd=lwds, lty=ltys, bty = "n" )

## logarithmic x and y axis
par(las=1)
plot(  density( var$ISAM ), col="red", type="n", lwd=2, xlab="GPP variance (kgC/m2/yr)", log="xy", xlim=c(0.5, 80), ylim=c(1e-4, 5e-1), main="" ) #
lapply( seq(length(modls)), function(x) lines( density( var[[ x ]] ), col=cols[x], lwd=lwds[x], lty=ltys[x] ) )
# legend( "topright", modls, col=cols, lwd=lwds, lty=ltys, bty = "n" )
...
## straight axes
par(las=1)
plot(  density( relvar$ISAM ), col="red", type="n", xlab="GPP relative variance (fraction)", ylim=c(0,0.25), xlim=c(0,60), main="" )
lapply( seq(length(modls)), function(x) lines( density( relvar[[ x ]] ), col=cols[x], lwd=lwds[x], lty=ltys[x] ) )
legend( "topright", modls, col=cols, lwd=lwds, lty=ltys, bty = "n" )

# ## logarithmic y axis
# par(las=1)
# plot(  density( relvar$ISAM ), col="red", type="n", lwd=2, xlab="GPP relative variance (fraction)", log="y", ylim=c(1e-6, 5e-1), xlim=c(0,60), main="" )
# lapply( seq(length(modls)), function(x) lines( density( relvar[[ x ]] ), col=cols[x], lwd=lwds[x], lty=ltys[x] ) )
# legend( "topright", modls, col=cols, lwd=lwds, lty=ltys, bty = "n" )

## logarithmic x and y axis
par(las=1)
plot(  density( relvar$ISAM ), col="red", type="n", lwd=2, xlab="GPP relative variance (fraction)", log="xy", ylim=c(1e-4, 2e-1), xlim=c(0.5,60), main="" ) #,
lapply( seq(length(modls)), function(x) lines( density( relvar[[ x ]] ), col=cols[x], lwd=lwds[x], lty=ltys[x] ) )
# legend( "topright", modls, col=cols, lwd=lwds, lty=ltys, bty = "n" )
